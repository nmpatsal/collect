    ...
    protected String doInBackground(Void... params) {
        synchronized (lock) {
            if (priority < lastPriorityUsed) {
                Timber.w("Savepoint thread (p=%d) was cancelled (a) because another one is waiting"
                        + "(p=%d)", priority, lastPriorityUsed);
                return null;
            }
            ...
            try {
                ...
                if (priority < lastPriorityUsed) {
                    Timber.w("Savepoint thread (p=%d) was cancelled (b) because another one is"
                            + "waiting (p=%d)", priority, lastPriorityUsed);
                    return null;
                }
